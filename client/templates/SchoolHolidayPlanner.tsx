...
import { Trash, Plus, CaretDown, CaretRight, Sun } from "phosphor-react-native";

const CATEGORIES = [
  { name: "Day Trip", emoji: "📍", color: "#3182CE" },
  { name: "Beach Day", emoji: "🏖️", color: "#0BC5EA" },
  { name: "Home Activity", emoji: "🏠", color: "#805AD5" },
  { name: "Educational", emoji: "📚", color: "#38A169" },
  { name: "Sports & Games", emoji: "⚽", color: "#DD6B20" },
  { name: "Social", emoji: "👥", color: "#D53F8C" },
  { name: "Adventure", emoji: "⛰️", color: "#E53E3E" },
  { name: "Nature", emoji: "🌳", color: "#319795" },
  { name: "Special Event", emoji: "✨", color: "#ECC94B" },
];
...
export default function SchoolHolidayPlanner({ listId }: { listId: string }) {
  ...
  const categorizedItems = useMemo(() => {
    ...
    todoIds.forEach((id) => { const cat = String(store?.getCell("todos", id, "category") || "Day Trip"); if (!grouped[cat]) grouped[cat] = []; grouped[cat].push(id); });
    return grouped;
  }, [todoIds, store]);

  const addItem = useAddRowCallback("todos", (data: any) => ({ text: data.text?.trim() || "", category: data.category || selectedCategory, emoji: CATEGORIES.find(c => c.name === (data.category || selectedCategory))?.emoji || "☀️", notes: "", date: "", done: false, list: listId }), [listId, selectedCategory]);
      ...
      <View style={styles.content}>
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            <Sun size={28} color="#DD6B20" weight="fill" />
            <View>
              <Text style={styles.title}>{String(listData?.name || "Holiday Planner")}</Text>
              <Text style={styles.subtitle}>{todoIds.length === 0 ? "Plan an epic holiday! ☀️" : `${todoIds.length} activities planned`}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.addSection}>
          ...
          <View style={styles.addRow}>
            <TextInput style={styles.addInput} placeholder="Add activity..." value={newTodo} onChangeText={setNewTodo} onSubmitEditing={handleAdd} placeholderTextColor="#A0AEC0" returnKeyType="done" />
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <View key={cat.name} style={styles.catSection}>
              ...
              {isOpen && <View style={styles.catItems}>{items.map((id) => <ActivityItem key={id} id={id} />)}</View>}
            </View>
          );
        })}
        ...
        {todoIds.length === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>Add activities for an amazing school holiday</Text>
          </View>
        )}
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  catChipSelected: { backgroundColor: "#ECC94B", borderColor: "#DD6B20" },
  ...
  itemDate: { fontSize: 11, color: "#DD6B20", marginTop: 2 },
